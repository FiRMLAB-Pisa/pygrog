...
Adaptive layout (2x2).  Top row shows per-call bars; bottom row shows
...
from _common import BENCHMARK_RESULTS_DIR, POSTER_STYLE, save_fig
...
def _bar_group(ax, x, offsets, series, w):
    ...
    *series* is a list of (vals, label, color) tuples aligned with *offsets*.
    ...
    """
    bars = []
    for off, (vals, label, color) in zip(offsets, series, strict=False):
...
def main() -> None:
    ...
    with POSTER_STYLE():
        ...
        for bars_c, (vals, *_) in zip(all_bars_rt, series_rt, strict=False):
            _annotate(ax, bars_c, vals, "{v:.1f} s")
        # CPU speedup callout (FINUFFT-CPU / PyGROG-CPU)
        for i, (vn, vg) in enumerate(zip([nu_fwd_rt, nu_adj_rt], [g_fwd_rt, g_adj_rt], strict=False)):
        ...
        ax.legend(frameon=False, fontsize=12, loc="upper right")
        for bars_c, (vals, *_) in zip(all_bars_mem, series_mem, strict=False):
        ...
        bars = ax.bar(labels, means, yerr=stds, capsize=6, color=["#2ca02c", "#ff7f0e"])
        ...
        fig.text(
            0.5, -0.01, "  ·  ".join(note_bits), ha="center", fontsize=12, color="#444"
        )
