...
def test_different_weighting(identity_interpolator, cartesian_2d_data):
    ...
    grog_distance = _ExtendedGrogInterpolator(
        coords=coords,
        shape=shape,
    ...
    grog_average = _ExtendedGrogInterpolator(
        coords=coords,
        shape=shape,
...
def test_kernel_setting_workflow():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        precomp_grog = _ExtendedGrogInterpolator(
            coords=coords,
            shape=shape
        )
        ...
        # Step 2: Runtime
        runtime_grog = _ExtendedGrogInterpolator.from_file(plan_path)
