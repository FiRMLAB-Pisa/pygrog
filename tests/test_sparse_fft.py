...
class TestPrimitives:
    """Exercise the C++ scatter_add / gather kernels with known inputs."""
    ...
    def test_gather_applies_weights(self):
...
class TestSparseFFTShape:
    """Output shapes on CPU and CUDA, with/without sensitivity maps."""

    def test_forward_no_smaps(self, device):
    ...
    def test_forward_with_smaps(self, device):
    ...
    def test_adjoint_no_smaps(self, device):
        op = _make_op((32, 32), (24, 24), 256)
    ...
    def test_adjoint_with_smaps(self, device):
        smaps = torch.randn(4, 24, 24, dtype=torch.complex64)
        op = _make_op((32, 32), (24, 24), 256, smaps=smaps)
    ...
    def test_call_adjoint_flag(self, device):
...
class TestSparseFFTAdjointness:
    """Dot-product adjoint test.  Exercises scatter_add + IFFT + crop path
    (forward) and FFT + pad + gather path (adjoint) via real computation."""

    def test_no_smaps(self, device):
    ...
    def test_with_smaps(self, device):
        torch.manual_seed(0)
        smaps = torch.randn(4, 24, 24, dtype=torch.complex64)
    ...
    def test_3d_adjointness(self, device):
...
class TestBatchHelpers:
    """_scatter_ifft_crop_batch and _fft_pad_gather_batch must produce
    the same values as calling the single-row path on each row in a loop."""

    def test_scatter_ifft_crop_batch_vs_loop(self, device):
    ...
    def test_fft_pad_gather_batch_vs_loop(self, device):
    ...
    def test_batch_helpers_are_adjoints(self, device):
