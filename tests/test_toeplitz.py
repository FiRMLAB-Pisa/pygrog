...
def _make_sparse_fft(
    grid_shape, n_samples, *, n_coils=None, osf=1, seed=0, toeplitz=True
):
    ...
    return SparseFFT(
        grid_shape, image_shape, indices, weights, smaps=smaps, toeplitz=toeplitz
    )
...
def test_sparse_fft_normal_matches_nested(osf, with_smaps):
    grid = (32 * osf, 32 * osf)
    op_t = _make_sparse_fft(
        grid, 600, n_coils=4 if with_smaps else None, osf=osf, toeplitz=True
    )
    op_n = _make_sparse_fft(
        grid, 600, n_coils=4 if with_smaps else None, osf=osf, toeplitz=False
    )
    ...
    if with_smaps:
        ...
        x = torch.randn(
            op_t.smaps.shape[0] if op_t.smaps is not None else 1,
            *image_shape,
            dtype=torch.complex64,
        )
    y_t = op_t.normal(x)
    y_n = op_n.adjoint(op_n.forward(op_n.adjoint(x))) if False else op_n.normal(x)
...
def test_orc_normal_matches_nested(osf):
    ...
    indices = torch.from_numpy(
        rng.integers(0, np.prod(grid), n_samples).astype(np.int64)
    )
    weights = torch.from_numpy((rng.random(n_samples).astype(np.float32) + 0.1))
    smaps = torch.from_numpy(
        (
            rng.standard_normal((4, *image_shape))
            + 1j * rng.standard_normal((4, *image_shape))
        ).astype(np.complex64)
        * 0.5
    )
    base = SparseFFT(grid, image_shape, indices, weights, smaps=smaps, toeplitz=True)
    base_n = SparseFFT(grid, image_shape, indices, weights, smaps=smaps, toeplitz=False)
    B = torch.from_numpy(
        (
            rng.standard_normal((n_samples, L))
            + 1j * rng.standard_normal((n_samples, L))
        ).astype(np.complex64)
    )
    C = torch.from_numpy(
        (
            rng.standard_normal((L, *image_shape))
...
def _make_subspace_base(grid, image_shape, T, n_pts, n_coils, *, toeplitz, seed=13):
...
def test_subspace_normal_matches_nested(osf):
    ...
    K = 3
    n_pts = 50
    base = _make_subspace_base(grid, image_shape, T, n_pts, n_coils=4, toeplitz=True)
    base_n = _make_subspace_base(grid, image_shape, T, n_pts, n_coils=4, toeplitz=False)
    rng = np.random.default_rng(17)
    basis = torch.from_numpy(
        (rng.standard_normal((K, T)) + 1j * rng.standard_normal((K, T))).astype(
            np.complex64
        )
    )
