...
class TestAutograd:
    ...
    def test_forward_produces_grad(self, device):
        op = _make_op()
        ksp = torch.randn(2, 64, dtype=torch.complex64, device=device,
                          requires_grad=True)
    ...
    def test_adjoint_produces_grad(self, device):
        op = _make_op()
        img = torch.randn(2, 12, 12, dtype=torch.complex64, device=device,
                          requires_grad=True)
    ...
    def test_forward_grad_nonzero(self, device):
    ...
    def test_adjoint_grad_nonzero(self, device):
        op = _make_op(seed=2)
        img = torch.randn(2, 12, 12, dtype=torch.complex64, device=device,
                          requires_grad=True)
